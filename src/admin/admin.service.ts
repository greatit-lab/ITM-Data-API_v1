...
export class AdminService {
  ...
  // ==========================================
  // [Usage Analytics] DB 실제 저장 및 조회 로직 (추가됨)
  // ==========================================
  ...
  async getUsageAnalytics(startDate: string, endDate: string) {
    ...
    const where = {
      accessTs: { gte: start, lte: end },
    };

    // 1. KPI 데이터
    const totalViews = await this.prisma.sysAccessLog.count({ where });
    const users = await this.prisma.sysAccessLog.groupBy({ by: ['loginId'], where });
    ...
    const dailyData = await this.prisma.$queryRaw`
      SELECT 
        to_char(access_ts, 'MM-DD') as date,
    ...
    return {
      kpi: { totalUsers, totalViews, topPage },
